...
def plot_normalized_results(original, norm_regular=None, norm_rolling=None, window_size=None, key_metrics=None):
    """
    ...
    if key_metrics is None:
        key_metrics = ['Close', 'Volume'] if all(metric in original.columns for metric in ['Close', 'Volume']) else original.columns[:2]
    ...
    if norm_rolling is not None:
        if window_size is None:
            raise ValueError("window_size must be provided when plotting rolling normalization")
        ...
        if 'original' in plots_to_show:
            fig.add_trace(
                go.Scatter(
                    ...
                    visible=True if visible else "legendonly"  # Show only key metrics by default
                ),
                row=plot_row, col=1
            )
            plot_row += 1
